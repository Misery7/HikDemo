...
public interface HCNetSDK extends StdCallLibrary {
    ...
    HCNetSDK HikInstance=(HCNetSDK)Native.loadLibrary("HCNetSDK",HCNetSDK.class);
     ...
     */
    public boolean NET_DVR_GetDVRConfig(NativeLong lUserID, int dwCommand, NativeLong lChannel, Pointer lpOutBuffer, int dwOutBufferSize, Pointer lpBytesReturned);
     ...
     */
    public int NET_DVR_Login_V40(HCNetSDK.NET_DVR_USER_LOGIN_INFO pLoginInfo, HCNetSDK.NET_DVR_DEVICEINFO_V40 lpDeviceInfo);
    ...
    public static class NET_DVR_CARD_CFG_V50 extends Structure //convert type int to int in Structure
    {
        public int dwSize;
        ...
        public byte byRes1;
        public byte[] byDoorRight=new byte[MAX_DOOR_NUM_256]; //door right (floor right), accord to bit, 1-has right 0-no right, from low bit to high bit means door 1-N have right
        public NET_DVR_VALID_PERIOD_CFG struValid; //valid period parameter
        public byte[] byBelongGroup = new byte[MAX_GROUP_NUM_128]; //Subordinate to the group, in public bytes, 1 - belongs to, 0 - does not belong to
        public byte[] byCardPassword = new byte[CARD_PASSWORD_LEN]; //card password
        public short[] wCardRightPlan = new short[MAX_DOOR_NUM_256]; //card right plan, value is from plan template No. use or method when same door has different plan template
        public int dwMaxSwipeTime; //max card time, 0 means infinite time
        public int dwSwipeTime; //has swiped card
        public short wRoomNumber;  //room number
        public short wFloorNumber;   //floor number
        public int dwEmployeeNo;   //employee no
        public byte[] byName = new byte[NAME_LEN];   //name
        public short wDepartmentNo;   //department no
        public short wSchedulePlanNo;   //schedule plan no
        public byte bySchedulePlanType;  //schedule plan type:0-no mean,1-personal,2-department
        public byte[] byRes2 = new byte[3];
        public int dwLockID;
        public byte[] byLockCode = new byte[MAX_LOCK_CODE_LEN];
        public byte[] byRoomCode = new byte[MAX_LOCK_CODE_LEN];
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[]{"dwSize","dwModifyParamType","byCardNo","byCardValid","byCardType","byLeaderCard","byRes1",
                    "byDoorRight","struValid","byBelongGroup","byCardPassword","wCardRightPlan","dwMaxSwipeTime","dwSwipeTime",
                    "wRoomNumber","wFloorNumber","dwEmployeeNo","byName","wDepartmentNo","wSchedulePlanNo","bySchedulePlanType",
                    "byRes2","dwLockID","byLockCode","byRoomCode","dwCardRight","dwPlanTemplate","dwCardUserId","byCardModelType","byRes3"});
        }
    }
    ...
    {
        public int dwSize;
        public int dwCardNum; //card number, 0xffffffff means to get all card information when getting
        public byte byCheckCardNo; //whether to verify card No. 0-not to verify, 1-verify
        public byte[] byRes1 = new byte[3];
        public short wLocalControllerID; //On-site controller serial number, said to the local controller issued offline card parameters, 0 is access control host
        public byte[] byRes2 = new byte[2];
        public int dwLockID;  //lock ID
        public byte[] byRes3 = new byte[20];
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[]{ "dwSize", "dwCardNum", "byCheckCardNo", "byRes1", "wLocalControllerID", "byRes2", "dwLockID", "byRes3"});
        }
    }
...
//bySupport5 &0x04support sence expend
    ...
    public static class NET_DVR_ACS_WORK_STATUS_V50 extends Structure
    {
        ...
        public byte[] byDoorStatus = new byte[MAX_DOOR_NUM_256];; //Door status(floor status), 1 - dormancy, 2 - normally open state, 3 - normally closed state, 4 - ordinary state
        public byte[] byMagneticStatus = new byte[MAX_DOOR_NUM_256];; //magnetic status 0 normally closed,1 normally open, 2 damage short - circuit alarm, 3 damage breaking alarm, 4 abnormal alarm
        public byte[] byCaseStatus = new byte[MAX_CASE_SENSOR_NUM];; //case status, 0-no input, 1-input
        ...
        public byte[] byCardReaderOnlineStatus = new byte[MAX_CARD_READER_NUM_512];; //card reader online status, 0-offline 1-online
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList(new String[]{ "dwSize","byDoorLockStatus","byDoorStatus","byMagneticStatus","byCaseStatus","wBatteryVoltage",
                    "byBatteryLowVoltage","byPowerSupplyStatus","byMultiDoorerlockStatus","byAntiSneakStatus","byHostAntiDismantleStatus",
                    "byIndicatorLightStatus","byCardReaderOnlineStatus","byCardReaderAntiDismantleStatus","byCardReaderVerifyMode","bySetupAlarmStatus",
                    "byAlarmInStatus","byAlarmOutStatus","dwCardNum","byFireAlarmStatus","byRes2"});
        }
    }
